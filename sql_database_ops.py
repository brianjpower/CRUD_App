...
class Crud_sql_ops:
    def __init__(self, enterID, enterName, enterDept, listBox):
        self.enterID = enterID
        self.enterName = enterName
        self.enterDept = enterDept
        self.id = None
        self.name = None
        self.dept = None
    ...
    def insertData(self):
        ...
        self.dept = self.enterDept.get()

        if self.id == "" or self.name == "" or self.dept == "":
            messagebox.showwarning("Cannot insert",
                                   "All three fields, employee id, employee name and employee dept are required")
        else:
            ...
            myCur.execute(sql, (self.id, self.name, self.dept))
            mydb.commit()
    ...
    def removeData(self):
        self.id = self.enterID.get().strip()
        if self.id == "":
        ...
        try:
            ...
            myCur.execute(del_query, (self.id,))
            if myCur.rowcount == 0:
                messagebox.showerror("Delete Error", f"No data found for Employee ID: {self.id}")
            else:
                # Complete the transaction
                mydb.commit()
                messagebox.showinfo("Delete Status", f"Data deleted successfully for employee {self.id}")
                # Clear the names filled by the user in the gui so it is ready for next operation
                show(self.listBox)
                self.enterID.delete(0, END)
                self.enterName.delete(0, END)
    ...
    def getData(self):
        self.id = self.enterID.get().strip()  # Get the id and remove any trailing characters
        ...
        try:
            ...
            myCur.execute(data_query, (self.id,))
            ...
            if result is None:
                ...
                self.enterName.delete(0, END)
                self.enterName.insert(0, empName)
                self.enterDept.delete(0, END)
                self.enterDept.insert(0, empDept)
    ...
    def updateData(self):
        ...
        if self.id == "" or self.name == "" or self.dept == "":
            ...
            myCur = mydb.cursor()
            check_query = "SELECT * FROM empDetails WHERE empID = %s"
            myCur.execute(check_query, (self.id,))
            result = myCur.fetchone()
            ...
            if result is None:  # If no record is found with the given empID
                ...
                myCur.execute(sql, (self.name, self.dept, self.id))
                mydb.commit()
                # Clear the names filled by the user in the gui so it is ready for next operation
                self.enterID.delete(0, END)
                self.enterName.delete(0, END)
